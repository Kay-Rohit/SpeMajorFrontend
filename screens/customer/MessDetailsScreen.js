...
function MessPageScreen({route}){
    ...
    const joinRequest = async() => {
        await axios.post(`${baseUrl}/customer/join/${user?.username}/${messData?.username}`,null,{
            headers:{
        ...
        <ScrollView>
...

            {
                ...
                (<>
                    <Card className="">
                        <Card.Content>
                            <Text className='capitalize' variant='titleLarge'>{messData?.messname}</Text>
                            <Text variant="titleMedium" className='capitalize mt-2'><Icon name='person'/> {messData?.firstname} {messData?.lastname}</Text>
                            <Text variant="titleMedium" 
                                onPress={() => {openDialer(messData?.phone)}}
                            >
                                <Icon name='phone'/> {messData?.phone}
                            </Text>
                            <Text variant="titleMedium" 
                                onPress={() => {openMaps(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(messData?.address)}`)}}
                            >
...

            {
                ...
                (
                    <View className="h-screen">
                        <View className='items-center p-5'><Text variant='titleMedium' className='uppercase'>This Week's Menu</Text></View>
                            {
